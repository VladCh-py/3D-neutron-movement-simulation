...
определяется при выполнении программы.
...
(или, в данном случае, в единичном интервале радиуса в цилиндрических координатах). Для этого в каждом цикле моделирования
сохраняются текущие координаты всех нейтронов, после чего строится гистограмма распределения.
Высота столбца гистограммы пропорциональна плотности нейтронов в данной области пространства,
что фактически отражает локальный поток нейтронов на расстоянии от оси (в конкретно данном примере, т.к. используется цилиндрическая геометрия).
